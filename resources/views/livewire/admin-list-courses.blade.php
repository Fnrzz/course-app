            ...
            <tbody>
                @if ($courses->count())
                    @foreach ($courses as $index => $course)
                        <tr>
                            <th scope="row">{{ $courses->firstItem() + $index }}</th>
                            <td>{{ $course->name }}</td>
                            <td>
                                <div class="d-flex">
                                    <a href="{{ route('adminDetailCourses', $course->id) }}"
                                        class="text-decoration-none">
                                        <span class="badge rounded-3 text-bg-info text-white">
                                            Detail
                                        </span>
                                    </a>
                                    <a href="{{ route('adminEditCourses', $course->id) }}" class="text-decoration-none">
                                        <span class="badge rounded-3 mx-2 text-bg-warning text-white">
                                            Edit
                                        </span>
                                    </a>
                                    <a href="{{ route('adminDeleteCourses', $course->id) }}"
                                        class="text-decoration-none">
